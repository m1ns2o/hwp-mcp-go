...
@mcp.tool()
def hwp_insert_image(path: str, width: int = None, height: int = None, use_original_size: bool = True,
                     max_width: int = None, max_height: int = None, scale: float = None,
                     keep_aspect_ratio: bool = False, embedded: bool = True, reverse: bool = False,
                     watermark: bool = False, effect: int = 0) -> str:
    """
    현재 커서 위치에 이미지를 삽입합니다. (통합된 이미지 삽입 기능)
    
    Args:
        path (str): 이미지 파일 경로 또는 URL
        width (int, optional): 이미지 너비 (hwpunit). keep_aspect_ratio=False일 때만 사용
        height (int, optional): 이미지 높이 (hwpunit). keep_aspect_ratio=False일 때만 사용
        use_original_size (bool): 원본 이미지 크기 사용 여부 (기본값: True)
        max_width (int, optional): 최대 너비 (hwpunit). keep_aspect_ratio=True일 때 사용
        max_height (int, optional): 최대 높이 (hwpunit). keep_aspect_ratio=True일 때 사용
        scale (float, optional): 비율 (예: 0.5 = 50%, 2.0 = 200%). keep_aspect_ratio=True일 때 사용
        keep_aspect_ratio (bool): 원본 비율 유지 여부 (기본값: False)
        embedded (bool): 이미지를 문서에 포함할지 여부 (기본값: True)
        reverse (bool): 좌우반전 여부 (기본값: False)
        watermark (bool): 워터마크로 설정할지 여부 (기본값: False)
        effect (int): 이미지 효과 (0: 실제그림, 1: 회색조, 2: 흑백, 기본값: 0)
    ...
    try:
        ...
        if hwp.insert_image(path, width, height, use_original_size, max_width, max_height,
                            scale, keep_aspect_ratio, embedded, reverse, watermark, effect):
            # 크기 정보 생성
            if use_original_size:
                size_info = "original size"
            elif keep_aspect_ratio:
                if scale is not None:
                    size_info = f"scale {scale}x (aspect ratio maintained)"
                elif max_width is not None or max_height is not None:
                    max_info = []
                    if max_width: max_info.append(f"max_width={max_width}")
                    if max_height: max_info.append(f"max_height={max_height}")
                    size_info = f"{', '.join(max_info)} (aspect ratio maintained)"
                else:
                    size_info = "aspect ratio maintained"
            else:
                actual_width = width if width is not None else "auto"
                actual_height = height if height is not None else "auto"
                size_info = f"{actual_width}x{actual_height}"

            # 효과 정보 추가
            effect_names = ["normal", "grayscale", "black&white"]
            effect_info = f", {effect_names[effect] if 0 <= effect < 3 else 'unknown'} effect"

            # 추가 옵션 정보
            options = []
            if reverse: options.append("reversed")
            if watermark: options.append("watermark")
            if not embedded: options.append("linked")
            options_info = f", {', '.join(options)}" if options else ""

            full_info = f"{size_info}{effect_info}{options_info}"
            logger.info(f"Successfully inserted image: {path} ({full_info})")
            return f"Image inserted successfully: {path} ({full_info})"
        else:
...
def hwp_insert_picture(
    ...
    """
    호환성을 위한 hwp_insert_picture 함수 (hwp_insert_image로 리다이렉트).
    
    Args:
        image_path (str): 이미지 파일 경로
        embedded (bool): 이미지를 문서에 포함할지 여부
        size_option (int): 크기 옵션 (0: 실제크기, 1: 지정크기)
        reverse (bool): 좌우반전 여부
        watermark (bool): 워터마크로 설정할지 여부
        effect (int): 이미지 효과 (0: 실제그림, 1: 회색조, 2: 흑백)
        width (int, optional): 이미지 너비 (hwpunit)
        height (int, optional): 이미지 높이 (hwpunit)
    
    Returns:
        str: 결과 메시지
    """
    use_original_size = (size_option == 0)
    return hwp_insert_image(
        path=image_path,
        width=width,
        height=height,
...
@mcp.tool()
def hwp_create_document_from_text(content: str, title: str = None, format_content: bool = True,
                                  save_filename: str = None, preserve_linebreaks: bool = True) -> dict:
    """
            ...
            try:
                if operation == "create":
                    ...
                    if not table_tools:
                        ...
                        if data:
                            resp = table_tools.create_table_with_data(rows, cols, json.dumps(data) if isinstance(data,
                                                                                                                 list) else data,
                                                                      has_header)
                            result["message"] = resp
...
@mcp.tool()
def hwp_insert_left_column() -> str:
    """현재 위치 왼쪽에 칼럼(열)을 삽입합니다."""
    try:
        table_tools = get_hwp_table_tools()
        if not table_tools:
            return "Error: Table tools not initialized"

        return table_tools.insert_left_column()
    except Exception as e:
        logger.error(f"왼쪽 칼럼 삽입 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


@mcp.tool()
def hwp_insert_right_column() -> str:
    ...
    try:
        ...
        return table_tools.insert_right_column()
    except Exception as e:
        logger.error(f"오른쪽 칼럼 삽입 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


@mcp.tool()
def hwp_insert_upper_row() -> str:
    ...
    try:
        ...
        return table_tools.insert_upper_row()
    except Exception as e:
        logger.error(f"위쪽 행 삽입 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


@mcp.tool()
def hwp_insert_lower_row() -> str:
    ...
    try:
        ...
        return table_tools.insert_lower_row()
    except Exception as e:
        logger.error(f"아래쪽 행 삽입 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


@mcp.tool()
def hwp_move_to_left_cell() -> str:
    """왼쪽 셀로 이동합니다."""
    try:
        table_tools = get_hwp_table_tools()
        if not table_tools:
        ...
        return table_tools.move_to_left_cell()
    except Exception as e:
        logger.error(f"왼쪽 셀로 이동 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


@mcp.tool()
def hwp_move_to_right_cell() -> str:
    """오른쪽 셀로 이동합니다."""
    try:
        table_tools = get_hwp_table_tools()
        if not table_tools:
        ...
        return table_tools.move_to_right_cell()
    except Exception as e:
        logger.error(f"오른쪽 셀로 이동 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


@mcp.tool()
def hwp_move_to_upper_cell() -> str:
    """위쪽 셀로 이동합니다."""
    try:
        table_tools = get_hwp_table_tools()
        if not table_tools:
        ...
        return table_tools.move_to_upper_cell()
    except Exception as e:
        logger.error(f"위쪽 셀로 이동 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


@mcp.tool()
def hwp_move_to_lower_cell() -> str:
    """아래쪽 셀로 이동합니다."""
    try:
        table_tools = get_hwp_table_tools()
        if not table_tools:
        ...
        return table_tools.move_to_lower_cell()
    except Exception as e:
        logger.error(f"아래쪽 셀로 이동 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


@mcp.tool()
def hwp_merge_table_cells() -> str:
    """선택된 셀들을 병합합니다."""
    try:
        table_tools = get_hwp_table_tools()
        if not table_tools:
        ...
        return table_tools.merge_table_cells()
    except Exception as e:
        logger.error(f"셀 병합 오류: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"


# @mcp.tool()
# def hwp_merge_cells_range(start_row: int, start_col: int, end_row: int, end_col: int) -> str:
...
#             return "Error: Table tools not initialized"
#
#         return table_tools.merge_cells_range(start_row, start_col, end_row, end_col)
#     except Exception as e:
#         logger.error(f"셀 범위 병합 오류: {str(e)}", exc_info=True)
#         return f"Error: {str(e)}"
#
#
# @mcp.tool()
# def hwp_merge_tables() -> str:
#     """표를 병합합니다."""
#     try:
#         table_tools = get_hwp_table_tools()
#         if not table_tools:
#             return "Error: Table tools not initialized"
#
#         return table_tools.merge_tables()
#     except Exception as e:
#         logger.error(f"표 병합 오류: {str(e)}", exc_info=True)
#         return f"Error: {str(e)}"
#
