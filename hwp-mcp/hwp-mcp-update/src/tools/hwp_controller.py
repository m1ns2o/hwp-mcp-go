...
class HwpController:
        ...
        try:
            ...
            if register_security_module:
                try:
                    ...
                    module_path = os.path.join(base_dir, "..", "..", "security_module",
    ...
    def run_action(self, action_name: str) -> bool:
        ...
        try:
            ...
            result = self.hwp.Run(action_name)
            # HWP Run commands return None for success, False for failure
            # Only return False if result is explicitly False, otherwise consider it success
            return result is not False
        except Exception as e:
            print(f"액션 실행 실패 ({action_name}): {e}")
            return False
    ...
    def insert_picture(self, image_path: str, embedded: bool = True, size_option: int = 1,
                       reverse: bool = False, watermark: bool = False, effect: int = 0,
                       width: int = None, height: int = None) -> bool:
        """
        호환성을 위한 insert_picture 메서드 (insert_image로 리다이렉트)
    ...
    def _get_image_dimensions(self, image_path: str) -> Tuple[int, int]:
        ...
        Returns:
            Tuple[int, int]: (width, height) 픽셀 단위
        """
        try:
            with Image.open(image_path) as img:
                return img.size
        except Exception as e:
            print(f"이미지 크기 정보를 가져올 수 없습니다: {e}")
            return (800, 600)  # 기본값
    ...
    def _download_image_from_url(self, url: str) -> str:
        ...
        try:
            ...
            print(f"이미지 다운로드 완료: {url} -> {temp_file.name}")
    ...
    def _calculate_proportional_size(self, original_width: int, original_height: int,
                                     max_width: int = None, max_height: int = None,
                                     scale: float = None) -> Tuple[int, int]:
        """
        비율을 유지하면서 크기를 계산합니다.
        
        Args:
            original_width (int): 원본 너비 (픽셀)
            original_height (int): 원본 높이 (픽셀)
            max_width (int, optional): 최대 너비 (hwpunit)
            max_height (int, optional): 최대 높이 (hwpunit)
            scale (float, optional): 비율 (예: 0.5 = 50%)
            
        Returns:
            Tuple[int, int]: (계산된 너비, 계산된 높이) 픽셀 또는 hwpunit
        """
        if scale is not None:
            # 스케일 기준 계산 - 원본 픽셀에 스케일만 곱함 (단순하고 정확)
            new_width = int(original_width * scale)
            new_height = int(original_height * scale)
            return (new_width, new_height)

        if max_width is not None or max_height is not None:
            # 최대 크기 기준으로 비율 계산 (hwpunit 단위)
            # 픽셀을 hwpunit으로 변환 (대략 1px = 26.458 hwpunit)
            px_to_hwpunit = 26.458
            hwp_width = original_width * px_to_hwpunit
            hwp_height = original_height * px_to_hwpunit

            if max_width is not None and max_height is not None:
                # 둘 다 지정된 경우, 더 제한적인 조건에 맞춤
                width_ratio = max_width / hwp_width
                height_ratio = max_height / hwp_height
                scale_ratio = min(width_ratio, height_ratio)
            elif max_width is not None:
                # 너비만 지정된 경우
                scale_ratio = max_width / hwp_width
            else:
                # 높이만 지정된 경우
                scale_ratio = max_height / hwp_height

            new_width = int(hwp_width * scale_ratio)
            new_height = int(hwp_height * scale_ratio)
            return (new_width, new_height)

        # 기본값 반환 (원본 픽셀 크기 그대로)
        return (original_width, original_height)

    def insert_image(self, image_path: str, width: int = None, height: int = None,
                     use_original_size: bool = True, max_width: int = None, max_height: int = None,
                     scale: float = None, keep_aspect_ratio: bool = False, embedded: bool = True,
                     reverse: bool = False, watermark: bool = False, effect: int = 0) -> bool:
        """
        현재 커서 위치에 이미지를 삽입합니다. (통합된 이미지 삽입 메서드)
        
        Args:
            image_path (str): 이미지 파일 경로 또는 URL
            ...
            embedded (bool): 이미지를 문서에 포함할지 여부 (True: 포함, False: 링크)
            reverse (bool): 좌우반전 여부
            watermark (bool): 워터마크로 설정할지 여부
            effect (int): 이미지 효과 (0: 실제그림, 1: 회색조, 2: 흑백)
        ...
        try:
            ...
            if image_path.startswith(('http://', 'https://')):
                # URL에서 이미지 다운로드
                temp_file_path = self._download_image_from_url(image_path)
            ...
            if use_original_size:
                # 원본 크기 사용 (size_option=0)
                result = self.hwp.InsertPicture(
                    abs_path, embedded, 0, reverse, watermark, effect, 0, 0
                )
            elif keep_aspect_ratio:
                # 비율 유지하면서 크기 계산
                try:
                    original_width, original_height = self._get_image_dimensions(abs_path)
                    calculated_width, calculated_height = self._calculate_proportional_size(
                        original_width, original_height, max_width, max_height, scale
                    )
                    result = self.hwp.InsertPicture(
                        abs_path, embedded, 1, reverse, watermark, effect, calculated_width, calculated_height
                    )
                except Exception as e:
                    print(f"비율 계산 중 오류 발생: {e}")
                    # 오류 시 원본 이미지 크기에서 계산된 기본 크기로 삽입
                    try:
                        original_width, original_height = self._get_image_dimensions(abs_path)
                        default_width, default_height = self._calculate_proportional_size(original_width,
                                                                                          original_height)
                        result = self.hwp.InsertPicture(
                            abs_path, embedded, 1, reverse, watermark, effect, default_width, default_height
                        )
                    except:
                        result = self.hwp.InsertPicture(
                            abs_path, embedded, 1, reverse, watermark, effect, 5000, 5000
                        )
            else:
                # 지정 크기 사용 (size_option=1)
                if width is None or height is None:
                    # 크기가 지정되지 않은 경우 원본 이미지에서 계산
                    try:
                        original_width, original_height = self._get_image_dimensions(abs_path)
                        default_width, default_height = self._calculate_proportional_size(original_width,
                                                                                          original_height)
                        actual_width = width if width is not None else default_width
                        actual_height = height if height is not None else default_height
                    except:
                        actual_width = width if width is not None else 5000
                        actual_height = height if height is not None else 5000
                else:
                    actual_width = width
                    actual_height = height

                result = self.hwp.InsertPicture(
                    abs_path, embedded, 1, reverse, watermark, effect, actual_width, actual_height
                )
